public class SquidEnemy extends Enemy{
    public SquidEnemy(Room room, int w, int h, int x, int y, int hp, int speed, int attack) {
        super(room, w, h, x, y, hp, speed, attack);
        animations=loadAnimations("Sprites/SquidEnemyAnimation");
        setAttack(new MindBolt(this));
    }
    @Override
    protected void movementLogic() {
        ...
        if(getTargetPos()!=null){
            ...
            int xSpeed=0;
            int ySpeed=0;
            if(getPosition().addVector(getWidth()/2,getHeight()/2).distance(getTargetPos().addVector(getRoom().getPlayer().getWidth()/2,getRoom().getPlayer().getHeight()/2))<750) {
                int enemyLeft = getX();
